import { Container, Row, Col, Button, Image } from "react-bootstrap";

import UniversityBanner from "../../../../assets/images/HowItWorks/universityBanner.jpg";

export default function UniversityHero() {
  const scrollToBenefits = () => {
    const benefitsSection = document.getElementById("university-benefits");
    if (benefitsSection) {
      benefitsSection.scrollIntoView({
        behavior: "smooth",
        block: "start",
      });
    }
  };
  return (
    <section className="hero-section">
      <div className="hero-overlay" />
      <Container className="hero-content">
        <Row className="justify-content-center">
          <Col lg={8} className="text-center">
            <h1 className="hero-title">For Universities</h1>
            <p className="hero-subtitle">
              Accept company share donations from alumni founders and grow your
              endowment with private equity gifts. We handle the complex legal,
              compliance, and custodial requirements so you can focus on impact.
            </p>
            <div className="hero-cta">
              <Button className="btn-primary-gradient me-3">Get Started</Button>
              <Button variant="outline-light" onClick={scrollToBenefits}>
                Learn More
              </Button>
            </div>
          </Col>
        </Row>
      </Container>
      <Image
        className="hero-image"
        src={UniversityBanner}
        alt="University Banner"
        fluid
      />
    </section>
  );
}
